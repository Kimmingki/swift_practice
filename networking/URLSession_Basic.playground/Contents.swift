...
struct Track: Codable {
    ...
    let thummnailPath: String
    
    enum CodingKeys: String, CodingKey {
        case title = "trackName"
        case artistName
        case thummnailPath = "artworkUrl100"
    }
}
...
let dataTask = session.dataTask(with: requestURL) { (data, response, error) in
    ...
    //  - Response, Track struct 두개 만들어야겠다
    ...
    do {
        ...
        print("---> tracks : \(tracks.count) - \(tracks.first?.title), \(tracks.first?.thummnailPath)")
    } catch let error {
